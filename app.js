...
async function fetchMpPayments() {
  ...
  try {
    ...
    function getTipo(mov) {
      // Se for transferência de saída (saque), classifica como saída
      if (mov.operation_type === "money_transfer") return "saída";
    ...
    // Limite ajustável pelo usuário
    let limit = 15;
    if (window.mpPaymentsLimit && window.mpPaymentsLimit.value) {
      if (window.mpPaymentsLimit.value === "all") limit = null;
      else limit = parseInt(window.mpPaymentsLimit.value, 10) || 15;
    }
    const pagamentosExibir = limit ? aReceber.slice(0, limit) : aReceber;
    tbody.innerHTML = pagamentosExibir.length
      ? pagamentosExibir
          .map(
            (mov) => {
              // Nome: busca em várias fontes
              let nome = "-";
              if (mov.account_holder_name) nome = mov.account_holder_name;
              else if (mov.payer && (mov.payer.first_name || mov.payer.last_name)) nome = `${mov.payer.first_name || ""} ${mov.payer.last_name || ""}`.trim();
              else if (mov.payer && mov.payer.identification && mov.payer.identification.long_name) nome = mov.payer.identification.long_name;
              else if (mov.collector && (mov.collector.first_name || mov.collector.last_name)) nome = `${mov.collector.first_name || ""} ${mov.collector.last_name || ""}`.trim();
              else if (mov.collector && mov.collector.long_name) nome = mov.collector.long_name;
              else if (mov.description) nome = mov.description;
              // E-mail: payer.email, collector.email
              let email = "-";
              if (mov.payer && mov.payer.email) email = mov.payer.email;
              else if (mov.collector && mov.collector.email) email = mov.collector.email;
              // Data
              let data = "-";
              if (mov.date_created) data = new Date(mov.date_created).toLocaleDateString("pt-BR");
              else if (mov.date_approved) data = new Date(mov.date_approved).toLocaleDateString("pt-BR");
              return `
        <tr>
          <td>${mov.id || "-"}</td>
      ...
      `;
          ...
          .join("")
      : '<tr><td colspan="7">Nenhum pagamento a receber.</td></tr>';
    if (aReceber.length > 15) {
      tbody.innerHTML += `<tr><td colspan="7" style="text-align:center;color:var(--muted);font-size:12px;">Exibindo apenas as ${limit || aReceber.length} mais recentes.</td></tr>`;
    // Atualiza a tabela ao mudar o seletor de limite
    window.mpPaymentsLimit = document.getElementById("mpPaymentsLimit");
    if (window.mpPaymentsLimit) {
      window.mpPaymentsLimit.addEventListener("change", fetchMpPayments);
    }
    }
  } catch (err) {
    tbody.innerHTML = "";
...
const syncExpiringNotifications = async (players) => {
  const tasks = [];
  // Busca notificações existentes para preservar o status 'read'
  const existingNotifs = await listNotifications(500);
  const notifReadMap = new Map();
  existingNotifs.forEach((n) => notifReadMap.set(n.id, !!n.read));
    ...
    if (player.vip?.ativo && shouldNotify(vipDaysLeft)) {
      ...
      tasks.push(
        upsertNotification(vipId, {
          ...
          read: notifReadMap.get(vipId) ?? false,
        }),
      );
    } else {
    ...
    if (player.seguro?.ativo && shouldNotify(seguroDaysLeft)) {
      ...
      tasks.push(
        upsertNotification(seguroId, {
          ...
          read: notifReadMap.get(seguroId) ?? false,
        }),
      );
    } else {
